...
import {ArrowRight,AudioWaveform,Check,ClipboardCopy,Command,CornerDownLeft,FolderOpen,GitBranch,Languages,LayoutGrid,LifeBuoy,Maximize2,Mic,MicOff,Minimize2,PanelBottomClose,PanelBottomOpen,Pencil,Plus,Redo2,RotateCcw,Save,Trash2,Waypoints,X} from "lucide-react";
...
import {diagramToNotation} from "@/lib/notation";
...
function Canvas({onLog}:{onLog:(l:Log)=>void}){
  ...
  return <LangContext.Provider value={lang}><JumpContext.Provider value={jumpTo}><main className="flex h-dvh min-h-[620px] flex-col overflow-hidden bg-[#07100e]">
    <header className="glass z-20 flex h-[72px] shrink-0 items-center justify-between border-b border-[#24352e] px-4 sm:px-7">
      ...
      <div className="flex items-center gap-2 sm:gap-4">{voice?<div className="hidden items-center gap-2 text-xs text-[#9bafa6] sm:flex"><span className="h-1.5 w-1.5 rounded-full bg-primary"/>{L.session}<span className="font-mono text-[#dce9e2]">{time}</span></div>:<span className="hidden text-xs text-[#71877d] sm:inline">{L.typeOnly}</span>}<Link href="/panduan" className="flex h-9 items-center gap-1.5 rounded-lg border border-[#2a3d35] bg-[#10201b] px-2.5 text-xs text-[#9bafa6] transition hover:border-primary/50 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"><LifeBuoy size={14}/><span className="hidden sm:inline">{L.guide}</span></Link><Select value={lang} onValueChange={v=>setLang(v as "id"|"en")}><SelectTrigger aria-label={L.langLabel} className={TRIGGER+" w-10 justify-center gap-0 px-0 [&>[data-slot=select-value]]:hidden [&>svg:last-child]:hidden sm:w-[156px] sm:justify-between sm:gap-2 sm:px-2.5 sm:[&>[data-slot=select-value]]:flex sm:[&>svg:last-child]:block"}><Languages size={14} className="shrink-0 text-[#71877d]"/><SelectValue className="text-[11px] leading-none"/></SelectTrigger><SelectContent align="end" className={MENU+" text-xs"}><SelectItem className="text-xs" value="id">Bahasa Indonesia</SelectItem><SelectItem className="text-xs" value="en">English</SelectItem></SelectContent></Select></div>
    </header>
